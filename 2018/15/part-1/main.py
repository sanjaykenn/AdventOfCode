	...
	for y, row in enumerate(inp.splitlines()):
		for x, cell in enumerate(row):
			if cell == '#':
				...
				goblins[y, x] = Character()

	battle = Battle(walls, elves, goblins)

	for round_id in itertools.count():
		battle.all_turns()

		if not battle.elves or not battle.goblins:
			total_hp = sum(elf.hp for elf in battle.elves.values()) + sum(goblin.hp for goblin in battle.goblins.values())
			return round_id * total_hp
