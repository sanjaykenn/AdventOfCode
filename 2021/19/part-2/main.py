...
def find_match(a, b):
	for p in itertools.product(a, b):
		shift_value = tuple(map(operator.sub, *p))
		new_beacons = shift(b, shift_value)
		if len(a & new_beacons) >= OVERLAP_COUNT:
			return new_beacons, shift_value
...
def main(inp):
	...
	while scanners_left:
		...
		if len(scanner_square_distances[index] & beacons_square_distances) >= overlapping_distances:
			for scanner in rotated_scanners[index]:
				scanner_match = find_match(beacons, scanner)
				if scanner_match is not None:
					new_beacons, shift_value = scanner_match
					beacons |= new_beacons
					scanners_left.popleft()
					beacons_square_distances |= {sq_distance(a, b) for a, b in itertools.product(beacons, new_beacons)}
					scanner_positions.add(shift_value)
					last_match = 0
					break
			else:
				last_match += 1
	...
	return max(sum(map(abs, map(operator.sub, a, b))) for a, b in itertools.combinations(scanner_positions, 2))
